...
% alternate rotation.
function [Normal_Proj_Data,optimal_N_packets_Values,j_opt]=Extract_alternate_rota_data_UCT(file_Address,N_packets,max_rot,num_peak,N_translation,N_rot,start_angle)
% file_Address;
% N_packets=3;
% max_rot=360;
% num_peak=10;
...
%         clf(figure(1));
        ...
        j_opt(j0,k+1)=j_opt0;
        optimal_N_packets_Values(j0,k+1,:)=mag_peak(j_opt0,:);%peak values  from optimal N_packet of ith rotation and j0th translation
        Normal_Proj_Data(j0,k+1)=max(mag_peak(1,:));%peak values  from First N_packet of ith rotation and j0th translation
    end
    k=k+1;
end
%save data.mat
